...
8. 【近期限售解禁】：若未来 14 个交易日内发生限售解禁，且占流通≥3%（或解禁市值≥50 亿且无占比数据），空仓口径 rating 不得为 Buy/Overweight；须等解禁落地后量价确认。
9. 【预约披露/除权除息】：未来 7 个交易日内有预约披露或除权除息时，提升不确定性权重；无明确 trigger 不追涨建仓。
"""

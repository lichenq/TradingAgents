...
def _build_national_holder_block(holder_block: Dict) -> Dict:
    """Structured national-fund holder events from holder_change news items."""
    items = (holder_block or {}).get("items") or []
    events = []
    for item in items:
        if not isinstance(item, dict):
            continue
        title = str(item.get("新闻标题") or item.get("title") or "")
        body = str(item.get("新闻内容") or item.get("content") or "")
        blob = f"{title} {body}"
        holder_type = ""
        for name in _NATIONAL_HOLDER_NAMES:
            if name in blob:
                holder_type = "大基金" if "集成电路" in name or "大基金" in name else (
                    "社保" if "社保" in name else ("汇金" if "汇金" in name else "证金")
                )
                break
        if not holder_type:
            continue
        direction = "reduce" if "减持" in blob else ("increase" if "增持" in blob else "unknown")
        if direction == "unknown":
            continue
        pct_m = re.search(r"持股比例?(?:降至|降为|下降至)?\s*(\d+(?:\.\d+)?)\s*%", blob)
        events.append(
            {
                "holder_label": holder_type,
                "holder_type": holder_type,
                "direction": direction,
                "is_planned_exit": any(k in blob for k in ("减持计划", "预披露", "预先披露", "计划减持")),
                "title": title[:200],
                "published_at": str(item.get("发布时间") or item.get("date") or ""),
                "pct_after": float(pct_m.group(1)) if pct_m else None,
            }
        )
    return {
        "category": "国家队/产业资本",
        "count": len(events),
        "reduction_count": sum(1 for e in events if e["direction"] == "reduce"),
        "increase_count": sum(1 for e in events if e["direction"] == "increase"),
        "events": events,
    }
...
def build_payload(code: str, stock_name: Optional[str], dates: List[str], limit: int, max_seconds: int, skip_sentiment: bool) -> Dict:
    ...
    nh_events = _build_national_holder_block(news_blocks["holder_change_buyback"])

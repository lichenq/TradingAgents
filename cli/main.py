...
def save_report_to_disk(final_state, ticker: str, save_path: Path):
    ...
    from tradingagents.graph.report_export import save_analysis_report_md
    from tradingagents.dataflows.report_paths import resolve_stock_display_name

    return save_analysis_report_md(
        final_state,
        ticker,
        save_path,
        trade_date=str(final_state.get("trade_date", "")),
        stock_name=resolve_stock_display_name(ticker),
    )
...
def run_analysis(checkpoint: bool = False):
    ...
    if save_choice in ("Y", "YES", ""):
        from tradingagents.dataflows.report_paths import build_report_bundle_name

        bundle = build_report_bundle_name(
            selections["ticker"], selections["analysis_date"]
        )
        default_path = Path(config["results_dir"]) / bundle

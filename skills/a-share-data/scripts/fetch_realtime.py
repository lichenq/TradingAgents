...
EM_FUND_FLOW_ALT_PATH = "/api/qt/stock/fflow/kline/get"
# push2his 常 Empty reply；push2delay 为东财备用行情域（ETF clist 等同源）
EM_FUND_FLOW_HOSTS = (
...
# 各源原始字段顺序不同，get_price 统一输出列：open, high, low, close, volume（索引为时间）
# 新浪 K 线：day, open, high, low, close, volume
# 腾讯 K 线：time, open, close, high, low, volume（注意：第 3 列是收盘，不是 high）


def _to_standard_ohlc_df(df: pd.DataFrame) -> pd.DataFrame:
    """保证列为 open/high/low/close，且 high/low 语义正确。"""
    need = {'open', 'high', 'low', 'close'}
    if not need.issubset(df.columns):
        raise ValueError(f"OHLC 列缺失: {list(df.columns)}")
    out = df[list(need) + ([c for c in ('volume',) if c in df.columns])].copy()
    for col in need:
        out[col] = pd.to_numeric(out[col], errors='coerce')
    if 'volume' in out.columns:
        out['volume'] = pd.to_numeric(out['volume'], errors='coerce')
  # 新浪列被误标为 open,close,high,low 时：close=原high, high=原low, low=原close
    o, h, l, c = out['open'], out['high'], out['low'], out['close']
    mis = (h < o) & (h < c) & (l > o) & (l <= c * 1.02)
    if mis.any():
        out.loc[mis, 'close'], out.loc[mis, 'high'], out.loc[mis, 'low'] = (
            l[mis], c[mis], h[mis]
        )
    hi = pd.concat([out['open'], out['close'], out['high'], out['low']], axis=1).max(axis=1)
    lo = pd.concat([out['open'], out['close'], out['high'], out['low']], axis=1).min(axis=1)
    out['high'] = hi
    out['low'] = lo
    return out
...
def _build_session() -> requests.Session:
    ...
    # Avoid IDE/sandbox HTTP_PROXY breaking domestic market APIs.
...
def _get_price_sina(code: str, count: int, frequency: str) -> pd.DataFrame:
    ...
    try:
        ...
        df = df.set_index('day')
        df.index.name = ''
        return _to_standard_ohlc_df(df)
    except Exception:
        return None
...
def _get_price_day_tx(code: str, count: int, frequency: str) -> pd.DataFrame:
    ...
    try:
        ...
        if not buf:
            return None
        # 腾讯日线字段顺序：time, open, close, high, low, volume
...
def _get_price_min_tx(code: str, count: int, frequency: str) -> pd.DataFrame:
    ...
    try:
        ...
        if not buf:
            return None
        # 腾讯分钟线字段顺序与日线一致：time, open, close, high, low, volume, ...
        ...
        if frequency == '1d':
            try:
                ...
                if ak_df is not None and not ak_df.empty:
                    ...
                    ak_df.index.name = ''
                    return _to_standard_ohlc_df(ak_df[['open', 'high', 'low', 'close', 'volume']])
            except Exception:
                pass
    elif frequency == '1m':
...
def _em_fund_flow_params(normalized: str) -> dict:
    clean = normalized[2:]
...
def _parse_em_fund_flow_klines(klines: list) -> pd.DataFrame:
    if not klines:
        return pd.DataFrame()
    ...
    numeric_cols = [c for c in temp_df.columns if c != "日期"]
    for col in numeric_cols:
        temp_df[col] = pd.to_numeric(temp_df[col], errors="coerce")
    return temp_df


def _em_fund_flow_headers(normalized: str) -> dict:
    return {
        "Referer": f"https://quote.eastmoney.com/{normalized}.html",
        "Origin": "https://quote.eastmoney.com",
    }


def _fetch_em_fund_flow_from_host(
    normalized: str, base_url: str, path: str
) -> pd.DataFrame:
    from http_util import get_json
    ...
    payload = get_json(
        url,
        params=_em_fund_flow_params(normalized),
        timeout=25,
        headers=_em_fund_flow_headers(normalized),
    )
    if not payload.get("data") or not payload["data"].get("klines"):
        return pd.DataFrame()
    return _parse_em_fund_flow_klines(payload["data"]["klines"])


def _fetch_em_fund_flow_df(normalized: str) -> tuple[pd.DataFrame, str]:
    errors = []
    best_df = pd.DataFrame()
    best_source = ""
    paths = (EM_FUND_FLOW_PATH, EM_FUND_FLOW_ALT_PATH)
    for host in EM_FUND_FLOW_HOSTS:
        host_tag = host.split("//", 1)[-1]
        for path in paths:
            path_tag = "daykline" if path.endswith("daykline/get") else "kline"
            label = f"{host_tag}/{path_tag}"
            try:
                df = _fetch_em_fund_flow_from_host(normalized, host, path)
                if df is None or df.empty:
                    continue
                if len(df) > len(best_df):
                    best_df = df
                    best_source = f"东方财富({label})"
            except Exception as exc:
                errors.append(f"{label}: {exc}")
    if not best_df.empty:
        return best_df, best_source
    detail = "; ".join(errors) if errors else "无可用数据"
    raise RuntimeError(detail)


def _fetch_fund_flow_df(normalized: str, market: str, clean: str) -> tuple[pd.DataFrame, str]:
    errors = []
    try:
        return _fetch_em_fund_flow_df(normalized)
    except Exception as exc:
        errors.append(str(exc))
    try:
        df = ak.stock_individual_fund_flow(stock=clean, market=market)
        if df is not None and not df.empty:
            return df, "东方财富(akshare)"
    except Exception as exc:
        errors.append(f"东方财富(akshare): {exc}")
    detail = "; ".join(errors) if errors else "无可用数据"
    raise RuntimeError(detail)


def _format_fund_flow_df(df: pd.DataFrame, days: int) -> pd.DataFrame:
...
def cmd_fund_flow(code: str, days: int, output_json: bool):
    ...
    clean = normalized[2:]
    try:
        df, source = _fetch_fund_flow_df(normalized, market, clean)
        raw_rows = len(df)
        df = _format_fund_flow_df(df, days)
        if raw_rows < days and not output_json:
            print(
                f"警告：请求近 {days} 日，实际仅 {raw_rows} 个交易日（push2his 异常时东财可能只返回最近一日）",
                file=sys.stderr,
            )
        if output_json:
            print(df.to_json(orient="records", force_ascii=False))
        else:
            print(f"【{code} 资金流向】近 {days} 日  数据源：{source}（单位：万元）")
            print(df.to_string(index=False))
    except Exception as e:
        print(f"获取资金流向失败：{e}")
        sys.exit(1)

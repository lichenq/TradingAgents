...
依赖已安装在：`~/.cursor/skills/a-share-data/.venv`（`akshare`、`MyTT`、`pandas`、`numpy`、`requests`）。

### Cursor / Agent 执行（必读）

Cursor Agent 终端会注入 `HTTP_PROXY=http://127.0.0.1:<端口>`，且 `requests` 对部分域名易出现 DNS/断连；**必须用 `run.sh`**（清代理 + 默认 `A_SHARE_PREFER_CURL=1`，HTTP 层 **curl 优先、requests 兜底**）。

**禁止**直接 `python3 scripts/...`。

```bash
SKILL_DIR="$HOME/.cursor/skills/a-share-data"
chmod +x "$SKILL_DIR/run.sh" "$SKILL_DIR/verify-agent.sh"
"$SKILL_DIR/verify-agent.sh"   # Agent 环境冒烟（quote/sector/fund-flow/kline）
"$SKILL_DIR/run.sh" fetch_realtime.py --quote 600519 --json
"$SKILL_DIR/run.sh" fetch_history.py --kline 600519 --start 2026-01-01 --end 2026-05-24 --freq d --json
"$SKILL_DIR/run.sh" fetch_technical.py 600519 --freq 1d --count 120 --indicators MA,MACD --json
"$SKILL_DIR/run.sh" fetch_stock_events.py --code 600519 --json
"$SKILL_DIR/run.sh" fetch_xueqiu_status.py --code 600519 --format json
"$SKILL_DIR/run.sh" fetch_ah_stocks.py --json
"$SKILL_DIR/run.sh" fetch_ah_ipo_timeline.py --code 600519 --json
"$SKILL_DIR/run.sh" fetch_sector_info.py --no-concepts 600519 --json
```

自检：`"$SKILL_DIR/verify-install.sh"`；脚本冒烟：`"$SKILL_DIR/run.sh" fetch_realtime.py --quote 600519 --json`。

`--fund-flow` 按域名轮询：`push2delay` → `push2his`（`http_util` curl/requests），最后 akshare；`push2his` 宕机时可能只返回最近 1 个交易日。

`fetch_sector_info.py` 行业/名称：**emweb 公司概况** → push2 → 新浪/腾讯名称 → akshare；全程经 `http_util`（curl 优先）。概念接口仍不稳定，请固定 `--no-concepts`。
...
- `fetch_xueqiu_status.py`：雪球 xueqiu.com 个股讨论帖（TradingAgents A 股情绪）
